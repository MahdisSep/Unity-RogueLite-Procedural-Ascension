...
public class PopulateDungeon : MonoBehaviour
{
    ...
    void SpawnBears()
    {
        ...
        foreach (Vertex vertex in vertices)
        {
            ...
            for (int i = 0; i < distances[vertex]; i++)
            {
                ...
                // if (mapData.FindRoom(vertex).Equals(mapData.FindRoom(mapData.end)))
                // {
                //     Instantiate(Bear, new Vector3(Random.Range(roomBounds.xMin + 2, roomBounds.xMax - 2), Random.Range(roomBounds.yMin + 2, roomBounds.yMax - 4)), Quaternion.identity);
                //     Instantiate(Bear, new Vector3(Random.Range(roomBounds.xMin + 2, roomBounds.xMax - 2), Random.Range(roomBounds.yMin + 2, roomBounds.yMax - 4)), Quaternion.identity);
    ...
    void SpawnSkeletrax()
    {
        ...
        var boss = Instantiate(enemywalk, new Vector3(room.center.x, room.center.y), Quaternion.identity);
        boss.GetComponent<MainEnemy>().room = room;
    }
